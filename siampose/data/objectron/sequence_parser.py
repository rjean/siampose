...
class ObjectronSequenceParser(torch.utils.data.Dataset):
    ...
    blurred_objects = [
        "bike"
    ]  # see https://github.com/google-research-datasets/Objectron/issues/10
    ...
    def __init__(
        ...
        assert os.path.exists(
            self.root_path
        ), f"invalid root dataset path: {self.root_path}"
        self.sequences_path = os.path.join(self.root_path, "sequences")
        assert os.path.exists(
            self.sequences_path
        ), f"invalid sequences path: {self.sequences_path}"
        if not objects:
            self.objects = self.all_objects
        else:
            assert all(
                [obj in self.all_objects for obj in objects]
        ...
        self.dataset = tf.data.TFRecordDataset(
            [s for shards in self.shards_map.values() for s in shards]
        )
...
class ObjectronFrameParser(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        assert all(
            [s in data for s in ["image/encoded", "point_2d", "point_3d"]]
        )  # there's more
    ...
    def __getitem__(self, idx):
        ...
        for (
            feat_name,
            feat_code,
        ) in selfsupmotion.data.objectron.schema.features.FEATURE_NAMES.items():
            assert feat_code in selfsupmotion.data.objectron.schema.features.FEATURE_MAP
            if feat_code not in self.data:
                continue
            feat_type = selfsupmotion.data.objectron.schema.features.FEATURE_MAP[
                feat_code
            ]
            ...
            if isinstance(feat_type, tf.io.FixedLenFeature):
                ...
                if feat_type.dtype == tf.string:
                    ...
                    if feat_name == "IMAGE_ENCODED":
                        feat_name = (
                            "IMAGE"  # overwrite feat name, it won't be encoded anymore
                        )
                        buf = np.frombuffer(
                            self.data[feat_code][idx].numpy(), dtype=np.uint8
                        )
                    ...
                    if (
                        feat_name == "TIMESTAMP_MCSEC"
                    ):  # manual handling for bugged spec
        ...
        im_scale = np.asarray(
            (out_sample["IMAGE"].shape[1], out_sample["IMAGE"].shape[0]), np.float32
        )
        for instance_idx, instance_point_count in enumerate(out_sample["POINT_NUM"]):
            instance_pts = frame_pts[
                point_idx_offset : point_idx_offset + instance_point_count
            ]
            bbox_min = tuple(
                np.multiply([instance_pts[:, i].min() for i in range(2)], im_scale)
            )
            bbox_max = tuple(
                np.multiply([instance_pts[:, i].max() for i in range(2)], im_scale)
            )
            centroid = tuple(
                np.multiply([instance_pts[:, i].mean() for i in range(2)], im_scale)
            )
