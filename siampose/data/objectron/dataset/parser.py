...
class ObjectronParser(object):
    ...
    def parse_example(self, example):
        ...
        image = self.get_image(
            fm[features.FEATURE_NAMES["IMAGE_ENCODED"]],
            shape=(self._in_width, self._in_height),
        )
        image = image / 255.0
    ...
    def parse_plane(self, example):
        ...
        fm = example.features.feature
        if (
            features.FEATURE_NAMES["PLANE_CENTER"] in fm
            and features.FEATURE_NAMES["PLANE_NORMAL"] in fm
        ):
