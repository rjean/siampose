...
def draw_annotation_on_image(image, object_annotations, num_keypoints):
    ...
    keypoints = [
        np.multiply(keypoint, np.asarray([w, h, 1.0], np.float32)).astype(int)
    ...
    for object_id in range(num_objects):
        ...
        for kp_id in range(num_keypoint):
            ...
            if not hidden[kp_id]:
                cv2.circle(
                    image,
                    (kp_pixel[0], kp_pixel[1]),
                    _POINT_RADIUS,
                    _COLORS[object_id % len(_COLORS)],
                    -1,
                )
