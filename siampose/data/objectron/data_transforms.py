...
class SimSiamFramePairTrainDataTransform(object):
    ...
    # @staticmethod
    def _generate_obj_crops(
        self, sample: typing.Dict, crop_height: typing.Union[int, str]
    ):
        """
        ...
        assert (
            ...
            and "IMAGE" in sample
            and "CENTROID_2D_IM" in sample
        )
        assert len(sample["IMAGE"].shape) == 4 and sample["IMAGE"].shape[1:] == (
            640,
            480,
            3,
        )
        assert (
            len(sample["CENTROID_2D_IM"].shape) == 2
            and sample["CENTROID_2D_IM"].shape[-1] == 2
        )
        ...
        if self.crop_strategy == "centroid":
            if isinstance(crop_height, int):
                tl = (
                    int(round(sample["CENTROID_2D_IM"][0, 0] - crop_height / 2)),
                ...
                max_size = (
                    max(real_br[0] - real_tl[0], real_br[1] - real_tl[1]) * 1.1
                )  # 10% extra
                tl = (
                    int(round(sample["CENTROID_2D_IM"][0, 0] - max_size / 2)),
            ...
            tl = (
                int(sample["POINTS"][0, :, 0].min()),
                int(sample["POINTS"][0, :, 1].min()),
            )
            br = (
                int(sample["POINTS"][0, :, 0].max()),
                int(sample["POINTS"][0, :, 1].max()),
            )
        else:
        ...
        if tl == br:  # should not happen!
            ...
            new_crop_height = 360
            tl = (
                int(round(sample["CENTROID_2D_IM"][0, 0] - new_crop_height / 2)),
                int(round(sample["CENTROID_2D_IM"][0, 1] - new_crop_height / 2)),
            )
            br = (tl[0] + new_crop_height, tl[1] + new_crop_height)
        output_crop_seq = []
        output_keypoints = []
        for frame_idx, (frame, kpts) in enumerate(
            zip(sample["IMAGE"], sample["POINTS"])
        ):
            ...
            if "POINTS" in sample:
                ...
                output_keypoints.append(
                    np.subtract(sample["POINTS"][frame_idx], offset_coords)
                )
        ...
        for i, obj_crop in enumerate(sample["OBJ_CROPS"]):
            ...
            assert (
                len(sample["OBJ_CROPS"][i]) > 0
            ), "Unable to crop image!"  # Don't allow return empty object!
        return sample
    ...
    def __call__(self, sample):
        ...
        sample["POINTS"] = [
            pts for pts in np.asarray(output_keypoints)[..., :2].astype(np.float32)
        ]
