...
def display_debug_frames(
    ...
    for instance_idx in range(sample_frame["INSTANCE_NUM"]):
        ...
        cv.circle(
            image,
            tuple(sample_frame["CENTROID_2D_IM"][instance_idx]),
            3,
            (255, 255, 255),
            -1,
        )
    ...
    for instance_idx in range(sample_frame["INSTANCE_NUM"]):
        ...
        for point_id in range(points_2d_new.shape[0]):
            cv.circle(
                image,
                (points_2d_new[point_id, 0], points_2d_new[point_id, 1]),
                10,
                (0, 255, 0),
                -1,
            )
...
def sort_sample_keypoints(
    ...
    assert all(
        [k in sample and len(sample[k]) == nb_frames_per_tuple for k in required_keys]
    )
    ...
    for frame_idx in range(nb_frames_per_tuple):
        ...
        plane_normal, plane_center = (
            sample["PLANE_NORMAL"][frame_idx],
            sample["PLANE_CENTER"][frame_idx],
        )
        ...
        good = good and (
            sum([np.isclose(dist, 0.0, atol=0.005) for dist in kpts_plane_dists]) == 4
        )
        ...
        sort_idxs_bottom, sort_idxs_top = np.argsort(kpts_cam_dists[:4]), np.argsort(
            kpts_cam_dists[4:]
        )
        kpts2d[:4], kpts3d[:4] = (
            kpts2d[:4][sort_idxs_bottom],
            kpts3d[:4][sort_idxs_bottom],
        )
        ...
        sample["POINT_3D"][frame_idx] = np.concatenate(
            [center3d.reshape((1, 3)), kpts3d]
        ).flatten()
