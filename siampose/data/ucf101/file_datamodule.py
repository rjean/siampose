...
class UCF101Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.classNamesByIndices = {
            idx: name for name, idx in self.classIndicesByNames.items()
        }
    ...
    def _get_sequences(self, category, split="train", fold=1):
        ...
        for splitId in splitIndices:
            seqDirectories = glob.glob(
                f"{self.root}/split-{splitId}/{partition}/class-{classId}/group-*/seq-*"
            )
            ...
            for sequence in self.sequences_by_categories[category]:
                ...
                if nbSamples > 5:
                    ...
                    for file in self.sequences_by_categories[category][sequence]:
                        ...
                        sample = {
                            "category": self.classNamesByIndices[classId],
                            "sequence": sequence,
                            "path": file,
                            "split": split,
                            "frame_id": frameId,
                    ...
                    print(
                        f"Skipping {category}/{sequence} : Not enough samples (%d)!"
                        % (nbSamples)
                    )
            print(
    ...
    def get_pair_of_filenames(self, sample, root):
        ...
        for i in range(0, 30):
            if self.pairing == "uniform":
                ...
                other_path = self.get_previous_filepath(
                ...
                if random.choice([True, False]):
                    other_path = self.get_next_filepath(
                        current_index, category, sequence
                    )
                else:
                    other_path = self.get_previous_filepath(
...
class UCF101FileDataModule(pytorch_lightning.LightningDataModule):
    ...
    def setup(self, stage=None):
        if not self.issetup:
            ...
            self.eval_transform = self.get_ucf101_transform(
            ...
            self.val_dataset = UCF101Dataset(
                self.data_dir,
                split="valid",
...
if __name__ == "__main__":
    ...
    dm = UCF101FileDataModule(
        data_dir="/data/sbrodeur/UCF-101/frames_112x112", num_workers=1
    )
    dm.setup()
