...
def extractFrames(
    ...
    outputDir="datasets/ucf101/frames_112x112",
    resolution=(112, 112),
):
    """
    ...
    for split in range(1, nbSplits + 1):
        for partition in ["train", "test"]:
            ...
            filenames = []
            with open(
                os.path.join(root, "%slist0%d.txt" % (partition, split)), "r"
            ) as f:
            ...
            for filename in tqdm.tqdm(filenames):
                ...
                dataType, className, groupId, seqId = os.path.splitext(
                ...
                try:
                    with imageio.get_reader(filename, "ffmpeg") as vid:
                        ...
                        for frameId, frame in enumerate(vid):
                            ...
                            imageio.imwrite(
                                os.path.join(
                                    outputFramesDir, "frame_%06d.jpg" % (frameId)
                                ),
                                frame,
                                quality=95,
                            )
                    ...
                    logging.error(
                        "Unable to properly extract frames for filename: %s"
                        % (filename)
                    )
                    traceback.print_exc()
...
if __name__ == "__main__":
    ...
    extractFrames(
        root="/data/sbrodeur/UCF-101",
        outputDir="/data/sbrodeur/UCF-101/frames_112x112",
        resolution=(112, 112),
    )
