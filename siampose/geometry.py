...
def get_dist_from_plane(
    plane_normal: np.ndarray, plane_center: np.ndarray, point: np.ndarray
) -> float:
...
def get_plane_equation_center_normal(
    plane_center: np.ndarray, plane_normal: np.ndarray
) -> np.ndarray:
...
def get_intersect_relative_to_camera(
    plane_normal: np.ndarray, plane_center: np.ndarray, point: np.ndarray
) -> np.ndarray:
...
def intersect_plane_with_line(
    plane_normal, plane_center, point1, point0=np.array([0, 0, 0])
):
    p0 = plane_center
    n = plane_normal
...
def snap_to_plane(
    ...
    new_center = intersect_plane_with_line(
        plane_normal_query, plane_center_query + offset, point_on_center_line
    )
...
def snap_box_to_plane(points3d, plane_normal, plane_center, align_axis=True):
    ...
    intersect = get_intersect_relative_to_camera(
        plane_normal, plane_center, bottom_middle
    )
...
def get_plane_points_z(
    scene_plane: np.ndarray, plane_center: np.ndarray, xmin=-3, ymin=-3, xmax=3, ymax=3
):
    ...
    plane_points_offset = np.vstack(
        [top_left - eps, bottom_left - eps, top_right - eps, bottom_right - eps]
    )
...
def find_face_facing_plane_v2(points, plane_normal):
    ...
    for face in FACES:
        ...
        closest_point_idx = int(
            np.argmin(
                np.dot(plane_normal, (points_from_current_face + plane_normal * 20).T)
            )
        )
        ...
        normalized = points_relative_to_closest_corner / np.linalg.norm(
            points_relative_to_closest_corner, axis=1
        ).reshape(4, 1)
        normalized[closest_point_idx] = np.array([0, 0, 0])
        dist = np.dot(
            plane_normal, (closest_corner + normalized + plane_normal * 20).T
        ).mean()
        if dist < min_dist:
...
def draw_bbox(
    ...
    points2d_px: list,
    line_color=(0, 255, 0),
    pixel_center_color=(0, 255, 0),
    object_center_color=(255, 0, 0),
    line_width=5,
):
