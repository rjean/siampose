...
from siampose.data.objectron.schema import (
...
def estimate_object_center_in_query_image(
    query_intrinsics, query_bbox, points_2d_px_result
):
...
def align_box_with_plane(points3d, plane_normal_query, plane_normal_result):
    box_rotation = geo.rotation_matrix_from_vectors(
...
def get_bounding_box(idx, match_idx, experiment, adjust_scale=False):
    ...
    plane_center_result, plane_normal_result = experiment.get_plane(
        match_idx, train=True
    )
    result_bbox = geo.get_bbox(points_2d_px_result, 360, 480)
    ...
    cx, cy, a, b = estimate_object_center_in_query_image(
        query_intrinsics, query_bbox, points_2d_px_result
    )
    center_ray = np.array([a, b, -1])
    points_3d_axis = align_box_with_plane(
        points_3d_result, plane_normal_query, plane_normal_result
    )
    obj_radius = geo.get_dist_from_plane(
        plane_normal_result, plane_center_result, points_3d_axis[0]
    )
    points_3d_result_snapped = geo.snap_to_plane(
        points_3d_axis, plane_normal_query, plane_center_query, center_ray, obj_radius
    )
    if adjust_scale:
        scale = get_smooth_scale_factor(
            query_bbox, points_3d_result_snapped, query_intrinsics, 2
        )
        points3d_scaled = points_3d_result_snapped
        for i in range(0, 4):
            # print(scale, obj_radius)
            obj_radius = geo.get_dist_from_plane(
                plane_normal_query, plane_center_query, points3d_scaled[0]
            )
            points3d_scaled = geo.snap_to_plane(
                geo.scale_3d_bbox(points3d_scaled, scale),
                plane_normal_query,
                plane_center_query,
                center_ray,
                obj_radius=obj_radius * scale,
            )
            scale = get_smooth_scale_factor(
                query_bbox, points3d_scaled, query_intrinsics, 2
            )
...
def get_match_aligned_points(
    ...
    points2d_valid_px = geo.points_2d_to_points2d_px(
        points2d_valid, valid_image.width, valid_image.height
    )
    points2d_train_px = geo.points_2d_to_points2d_px(
        points2d_train, train_image.width, train_image.height
    )
    ...
    points3d_train_rotated, _ = geo.align_with_bbox_3d(
        points3d_train, train_bbox, valid_bbox
    )
    if ground_truth:
...
def get_match_snapped_points(
    ...
    points3d_result_rotated, points3d_result = get_match_aligned_points(
        idx, match_idx, info_df, train_info_df, ground_truth=ground_truth
    )
    snapped, intersect = geo.snap_box_to_plane(
        points3d_result_rotated, plane_normal, plane_center, align_axis=align_axis
    )
    ...
    if rescale:
        ...
        valid_bbox = geo.get_bbox(
            points2d_valid_px, valid_image.width, valid_image.height
        )
        ...
        snapped_rotated, intersect = geo.snap_box_to_plane(
            snapped_rotated, plane_normal, plane_center
        )
...
def get_iou(
    ...
    if ground_plane:
        points3d_processed, points3d_valid = get_match_snapped_points(
            idx,
            match_idx,
            info_df,
            train_info_df,
        ...
        points3d_processed, points3d_valid = get_match_aligned_points(
            idx, match_idx, info_df, train_info_df, ground_truth=True
        )
...
def find_match_idx(
    idx, query_embeddings: np.ndarray, train_embeddings: np.ndarray, k=0, score=False
):
...
def find_all_match_idx(query_embeddings: np.ndarray, train_embeddings: np.ndarray, k=0):
    ...
    if k != 0:
        ...
        for i in tqdm(range(0, math.ceil(len(query_embeddings) / query_chunk_size))):
            ...
            best_match_idx_chunk = np.zeros(
                (query_end - query_start, 1), dtype=np.uint64
            )
            for j in range(0, math.ceil(train_embeddings.shape[1] / train_chunk_size)):
                ...
                match_idx_chunk_score = np.take_along_axis(
                    similarity, np.expand_dims(match_idx_chunk, axis=1), axis=1
                )
                ...
                best_match_idx_chunk = np.where(
                    ...
                    best_match_idx_chunk,
                ).astype(np.uint64)
                best_match_idx_chunk_score = np.where(
                    match_idx_chunk_score > best_match_idx_chunk_score,
                    match_idx_chunk_score,
                    best_match_idx_chunk_score,
                )
...
def get_iou_mp(
    idx: int,  # , symmetric=False, rescale=False,
    ...
    if args.random_bbox_same:
        match_idx = random.sample(
            list(experiment.train_info_df.query(f"category=='{category}'").index), 1
        )
...
class ExperimentHandlerFile:
    ...
    def read_experiment(self, experiment: str, test: bool):
        ...
        if test:
            info_prefix = "test_"
            print(
                "Loading experiment with test set instead of validation set embeddings!"
            )
        ...
        try:
            if not self.test:
                with open(f"{experiment}/train_sequences.txt", "r") as f:
                    ...
                    valid_sequences_g = sorted(
                        list((self.info_df["sequence_uid"].unique()))
                    )
                    for valid_sequence in valid_sequences_g:
                        if valid_sequence in train_sequences_t:
                            raise ValueError(
                                f"The validation sequence {valid_sequence} was part of the training set!"
                            )
        except FileNotFoundError:
            print(
                f"Warning, unable to find {experiment}/train_sequences.txt.\n We will not be able to make sure there is no overlap between training and validation!"
            )

    def load_hdf5_file(self):
        if self.hdf5_dataset is None:
            self.hdf5_dataset = h5py.File(
                "/home/raphael/datasets/objectron/extract_s5_raw.hdf5", "r"
            )
        if self.test and self.hdf5_test_dataset is None:
            self.hdf5_test_dataset = h5py.File(
                "/home/raphael/datasets/objectron/extract_s5_raw_test.hdf5", "r"
            )
    ...
    def get_points(self, idx: int, train=False):
        if self.mode == "raw":
            ...
            image_idx = list(hdf5_dataset[category][sequence]["IMAGE_ID"]).index(
                image_id
            )
            return hdf5_dataset[category][sequence]["POINT_2D"][image_idx].reshape(
                9, 3
            ), hdf5_dataset[category][sequence]["POINT_3D"][image_idx].reshape(9, 3)
        else:
    ...
    def _raw_get_points(self, idx: int, train=False):
        ...
        idx = fix_idx(idx)
        sequence_annotations = self._raw_get_sequence_annotations(
            df.iloc[idx]["category"],
            df.iloc[idx]["batch_number"],
            df.iloc[idx]["sequence_number"],
        )
        frame = int(df.iloc[idx]["frame"])
        object_id = int(df.iloc[idx]["object_id"])
        keypoints = (
            sequence_annotations.frame_annotations[frame]
    ...
    def _raw_get_sequence_annotations(
        self,
        category: str,
        batch_number: str,
        sequence_number: str,
        annotations_path="/home/raphael/datasets/objectron/annotations",
    ):
        ...
        annotation_file = (
            f"{annotations_path}/{category}/{batch_number}/{sequence_number}.pbdata"
        )
    ...
    def get_plane(self, idx: int, train=False):
        if self.mode == "raw":
            ...
            return (
                hdf5_dataset[category][sequence]["PLANE_CENTER"][image_idx],
                hdf5_dataset[category][sequence]["PLANE_NORMAL"][image_idx],
            )
        else:
            raise ValueError(f"Mode not supported: {self.mode}")
    ...
    def get_intrinsics(self, idx, train=False):
        if self.mode == "raw":
            ...
            return hdf5_dataset[category][sequence]["INTRINSIC_MATRIX"][
                image_idx
            ].reshape(3, 3)
        else:
            raise ValueError(f"Mode not supported: {self.mode}")
    ...
    def _get_subset(info_df, embeddings, ratio):
        ...
        print(
            f"Using a subset of {len(videos_uids_subset)} out of {len(video_uids)} total sequences for evaluation."
        )
    ...
    def set_trainsubset(self, ratio):
        train_info_df, train_embeddings = self._get_subset(
            self.train_info_df, self.train_embeddings, ratio
        )
        self.train_info_df = train_info_df
        self.train_embeddings = train_embeddings
    ...
    def parse_info_df(self, info_df, subset="valid"):
        ...
        info_df["filepath"] = (
            f"/home/raphael/datasets/objectron/96x96/{subset}/"
            + info_df["category"]
            + "/"
        ...
        info_df["filepath_full"] = (
            "/home/raphael/datasets/objectron/640x480_full/"
            + info_df["category"]
            + "/"
            + info_df["sequence_uid"]
            + "."
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Command line tool for evaluating zero shot pose estimation on objectron."
    )
    parser.add_argument(
        "experiment",
        type=str,
        help="Experiment folder location. i.e. ../SimSiam/outputs/objectron_96x96_experiment_synchflip_next",
    )
    parser.add_argument(
        "--subset_size",
        type=int,
        default=1000,
        help="Number of samples for 3D IoU evaluation",
    )
    parser.add_argument(
        "--ground_plane", default=True, help="If enabled, snap to ground plane"
    )
    parser.add_argument(
        "--iou_t",
        default=0.5,
        help="IoU threshold required to consider a positive match",
    )
    parser.add_argument(
        "--symmetric",
        action="store_true",
        help="Rotate symetric objects (cups, bottles) and keep maximum IoU.",
    )
    parser.add_argument(
        "--rescale", action="store_true", help="Rescale 3d bounding box"
    )
    parser.add_argument(
        "--random_bbox",
        action="store_true",
        help="Fit a randomly selected bbox instead of the nearest neighbor",
    )
    parser.add_argument(
        "--random_bbox_same",
        action="store_true",
        help="Fit a randomly selected bbox from same category instead of the nearest neighbor",
    )
    parser.add_argument(
        "--trainset-ratio",
        type=float,
        default=1,
        help="Ratio of the training set sequences used for inference",
    )
    parser.add_argument(
        "--single_thread", action="store_true", help="Disable multithreading."
    )
    parser.add_argument("--cpu", action="store_true", help="Disable cuda accelaration.")
    parser.add_argument(
        "--no_align_axis",
        action="store_true",
        help="Don't to to align axis with ground plane.",
    )
    parser.add_argument(
        "--legacy", action="store_true", help="Deprecated legacy evalution mode"
    )
    parser.add_argument(
    ...
    all_match_idxs = find_all_match_idx(
        experiment.embeddings, experiment.train_embeddings, 0
    )
    # get_iou_mp(1)
    if args.subset_size < len(experiment.info_df):
        ...
        print(
            f"Evaluating on all samples size subset size ({args.subset_size}) is larger that test set size ({len(experiment.info_df)})."
        )
    ...
    for category in sorted(ious.columns):
        ...
        print(
            f"{category: <10}\t{column.mean():0.2f}\t\t{column.median():0.2f}\t\t{ap_at_iout:0.2f}"
        )
